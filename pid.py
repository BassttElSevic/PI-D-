...
try:
    from PyQt5.QtWidgets import (
        ...
        QLabel, QSlider, QPushButton, QLineEdit, QGroupBox, QFormLayout, QMessageBox, QSpinBox, QCheckBox
    )
...
def resource_path(relative_path: str) -> str:
    """
    在开发环境和 PyInstaller 打包后的运行环境下，返回资源文件的绝对路径。
    """
    # PyInstaller 在运行时会把临时解包目录放在 sys._MEIPASS
...
class PIController:
    ...
    def __init__(self, Kp: float, Ki: float, Ts: float,
        ...
        参数：
            ...
            u_min: 控制输出最小值限制
            u_max: 控制输出最大值限制
            Kd: 微分增益系数（对测量微分，减少设定值突变带来的微分突跳）
        """
        self.Kp = float(Kp)
        self.Ki = float(Ki)
        self.Kd = float(Kd)
        ...
        self.I = 0.0                 # 积分项状态
        self.last_u = 0.0            # 上一次输出
        self.last_y = None           # 上一次测量值（用于D项，首次为None）
    ...
    def step(self, r: float, y: float) -> float:
        ...
        # D项（对测量微分，避免设定值阶跃引起的微分突跳）
        if self.last_y is None or self.Ts <= 0:
            D = 0.0
        else:
            dy = y - self.last_y
            D = - self.Kd * (dy / self.Ts)

        # I项尝试更新
        I_try = self.I + self.Ki * self.Ts * e

        # 原始输出
        u_raw = self.Kp * e + I_try + D

        # 限幅
        u_sat = clamp(u_raw, self.u_min, self.u_max)

        # Anti-windup 条件：当饱和且误差方向推向更饱和，则不积
        saturated = (u_raw != u_sat)
        pushing_harder = ((u_raw > u_sat and e > 0) or (u_raw < u_sat and e < 0))
        if not (saturated and pushing_harder):
            self.I = I_try
...
class PIDControlUI(QMainWindow):
    ...
    def __init__(self):
        ...
        # QSettings 持久化设置
        self.settings = QSettings('PIDDemo', 'PIDControlApp')
        ...
        if hasattr(self, 'dark_checkbox'):
            self.dark_checkbox.setChecked(bool(self.dark_mode))
        if hasattr(self, 'linewidth_spin'):
            self.linewidth_spin.setValue(int(self.line_width))
    ...
    def apply_app_font(self):
        """应用全局界面字体大小。"""
        try:
            QApplication.setFont(QFont('', int(self.font_size)))
        except Exception:
            pass

    def apply_app_palette(self):
        """应用浅色/深色模式到 Qt 应用。"""
        try:
            if self.dark_mode:
    ...
    def save_settings(self):
        """保存用户设置到本地。"""
        try:
            self.settings.setValue('ui/font_size', int(self.font_size))
            self.settings.setValue('ui/dark_mode', bool(self.dark_mode))
            self.settings.setValue('ui/line_width', int(self.line_width))
            self.settings.sync()
        except Exception:
            pass
    ...
    def load_settings(self):
        """从本地加载用户设置。"""
        try:
            fs = self.settings.value('ui/font_size', 12, type=int)
            dm = self.settings.value('ui/dark_mode', False, type=bool)
            lw = self.settings.value('ui/line_width', 2, type=int)
            if isinstance(fs, int):
                self.font_size = fs
            if isinstance(dm, bool):
                self.dark_mode = dm
            if isinstance(lw, int) and lw > 0:
                self.line_width = lw
        except Exception:
            pass
    ...
    def create_control_panel(self):
        ...
        kd_layout.addWidget(QLabel("微分系数 Kd:"))
        ...
        self.kd_input.textChanged.connect(self.update_kd_from_input)
        kd_layout.addWidget(self.kd_input)
        kd_layout.addWidget(self.kd_slider)
        control_layout.addLayout(kd_layout)
        ...
        ui_layout.addRow(QLabel("字体大小 (pt):"), self.font_spin)
        # 反色（深色模式）
        self.dark_checkbox = QCheckBox("深色模式")
        self.dark_checkbox.setChecked(bool(self.dark_mode))
        self.dark_checkbox.toggled.connect(self.update_dark_mode)
        ui_layout.addRow(self.dark_checkbox)
        # 线宽
        self.linewidth_spin = QSpinBox()
        self.linewidth_spin.setRange(1, 10)
        self.linewidth_spin.setValue(int(self.line_width))
        self.linewidth_spin.setSingleStep(1)
        self.linewidth_spin.valueChanged.connect(self.update_line_width)
        ui_layout.addRow(QLabel("线宽 (px):"), self.linewidth_spin)
    ...
    def create_plot_area(self):
        """创建图表显示区域（Matplotlib + Qt5Agg 画布）。"""
        from matplotlib.backends.backend_qt5agg import FigureCanvasQTAgg
        # 初始图背景按当前模式设置
        face = '#111111' if getattr(self, 'dark_mode', False) else '#FFFFFF'
        self.figure = plt.Figure(figsize=(5, 4), dpi=100, facecolor=face)
        self.canvas = FigureCanvasQTAgg(self.figure)

    def init_parameters(self):
        """初始化界面参数（同步滑块/输入框/SpinBox 的默认状态）。"""
        # 同步 Kp
        try:
            kp = float(self.kp_input.text()) if hasattr(self, 'kp_input') else 10.0
            if hasattr(self, 'kp_slider'):
                self.kp_slider.setValue(int(kp * 10))
        except Exception:
            pass
        # 同步 Ki
        try:
            ki = float(self.ki_input.text()) if hasattr(self, 'ki_input') else 0.5
            if hasattr(self, 'ki_slider'):
                self.ki_slider.setValue(int(ki * 100))
        except Exception:
            pass
        # 同步 Kd
        try:
            kd = float(self.kd_input.text()) if hasattr(self, 'kd_input') else 0.0
            if hasattr(self, 'kd_slider'):
    ...
    def update_kp_from_slider(self, value: int):
        """根据Kp滑块更新输入框。"""
        try:
            kp_value = value / 10.0
    ...
    def update_kp_from_input(self, text: str):
        """根据Kp输入框更新滑块。"""
        try:
            value = float(text)
    ...
    def update_ki_from_slider(self, value: int):
        """根据Ki滑块更新输入框。"""
        try:
    ...
    def update_ki_from_input(self, text: str):
        """根据Ki输入框更新滑块。"""
        try:
            value = float(text)
    ...
    def update_kd_from_slider(self, value: int):
        """根据滑块值更新Kd输入框"""
        try:
            kd_value = value / 100.0
            self.kd_input.setText(str(kd_value))
        except Exception:
            pass

    def update_kd_from_input(self, text: str):
        """根据输入框值更新Kd滑块"""
        try:
            value = float(text)
    ...
    def update_font_size(self, value: int):
        ...
        self.apply_app_font()
        self.save_settings()
        # 重新绘图以应用到 Matplotlib 文本
        try:
            self.run_simulation_and_plot()
        except Exception:
            pass

    def update_dark_mode(self, checked: bool):
        """用户切换深色模式。"""
    ...
    def update_line_width(self, value: int):
        """用户调节线宽。"""
        try:
    ...
    def plot_results(self, res, params):
        ...
        # 计算各类字体大小
        title_fs = max(8, int(self.font_size) + 2)
        label_fs = max(8, int(self.font_size))
        tick_fs = max(6, int(self.font_size) - 1)
        legend_fs = max(6, int(self.font_size) - 1)

        lw = max(1, int(self.line_width))

        # 温度曲线
        l1, = ax1.plot(res["t"], res["y"], label="实际温度", color="#1f77b4", linewidth=lw)
        l2, = ax1.plot(res["t"], res["r"], label="目标温度", color="#ff7f0e", linestyle="--", linewidth=lw)
        l3, = ax1.plot(res["t"], res["ambient"], label="环境温度", color="#2ca02c", alpha=0.7, linewidth=lw)
        ax1.set_ylabel("温度 (°C)", fontsize=label_fs, color=text_color)
        ax1.set_title(f"温度控制效果 (Kp={params['kp']}, Ki={params['ki']}, Kd={params.get('kd', 0.0)})", fontsize=title_fs, color=text_color)
        leg1 = ax1.legend()
        if leg1:
            leg1.get_frame().set_alpha(0.1 if self.dark_mode else 0.8)
            for txt in leg1.get_texts():
                txt.set_color(text_color)

        # 控制输出曲线
        ax2.plot(res["t"][:-1], res["u"], label="控制输出", color="#d62728", linewidth=lw)
        ax2.set_ylabel("控制输出 (%)", fontsize=label_fs, color=text_color)
        ax2.set_title("控制器输出", fontsize=title_fs, color=text_color)
        leg2 = ax2.legend()
        if leg2:
            leg2.get_frame().set_alpha(0.1 if self.dark_mode else 0.8)
            for txt in leg2.get_texts():
                txt.set_color(text_color)

        # 误差曲线
        ax3.plot(res["t"][:-1], res["e"], label="控制误差", color="#9467bd", linewidth=lw)
        ax3.set_ylabel("误差 (°C)", fontsize=label_fs, color=text_color)
        ax3.set_xlabel("时间 (s)", fontsize=label_fs, color=text_color)
        ax3.set_title("控制误差", fontsize=title_fs, color=text_color)
        leg3 = ax3.legend()
        if leg3:
            leg3.get_frame().set_alpha(0.1 if self.dark_mode else 0.8)
            for txt in leg3.get_texts():
                txt.set_color(text_color)

        # 刻度字号（颜色已在 tick_params 设置）
        for ax in (ax1, ax2, ax3):
